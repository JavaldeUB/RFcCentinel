...
def alinear_y_generar_espectro_sintetico(frecuencias, s21_list, frecuencias_min, freq_promedio_min):
    ...
    for i in range(len(frecuencias)):
        ...
        interpolador = interp1d(frecuencias[i], s21_list[i], kind='linear', fill_value="extrapolate")        
        interpolador1 = interp1d(frec_ajustada, s21_list[i], kind='linear', fill_value="extrapolate")
        s21_interpolado = interpolador1(freq_comun)
        s21_interpoladoOld = interpolador(freq_comun)
        # Guardar los espectros alineados e interpolados
        s21_interpolados.append(s21_interpoladoOld)
    ...
    s21_interpolados = np.array(s21_interpolados)
    
    # Promediar los espectros alineados para generar el espectro sintético
    s21_sintetico = np.mean(s21_alineados, axis=0)
    diff = abs(s21_interpolados-s21_sintetico)**2
    var = np.sqrt(diff.sum()/(len(s21_alineados)-0))
    print(f"Standard Deviation of the spectra: {var}")
