...
from PyQt5.QtWidgets import (
    ...
    QComboBox, QTextEdit, QWidget, QFileDialog, QLineEdit
)
...
class SerialApp(QMainWindow):
    def __init__(self):
        ...
        # New Textbox for number of readings and filename prefix
        self.num_readings_textbox = QLineEdit()
        self.num_readings_textbox.setPlaceholderText("Number of readings (e.g. 5)")
        
        self.filename_prefix_textbox = QLineEdit()
        self.filename_prefix_textbox.setPlaceholderText("Filename prefix")
        ...
        main_layout.addWidget(QLabel('Number of Readings:'))
        main_layout.addWidget(self.num_readings_textbox)
        main_layout.addWidget(QLabel('Filename Prefix:'))
        main_layout.addWidget(self.filename_prefix_textbox)
    ...
    def send_command(self):
        """Send the command from the command textbox to the serial port N times."""
        if self.serial_port and self.serial_port.is_open:
            command = self.command_textbox.toPlainText().strip()
            num_readings = self.num_readings_textbox.text().strip()
            filename_prefix = self.filename_prefix_textbox.text().strip()

        # Validate inputs
            if not command:
                self.response_textbox.append("Command is empty.")
                return

            if not num_readings.isdigit():
                self.response_textbox.append("Please enter a valid number of readings.")
                return

            num_readings = int(num_readings)

            # Send the command N times and save the results
            for i in range(num_readings):
                self.serial_port.write((command + '\n').encode('utf-8'))
                self.response_textbox.append(f"Sent command: {command} ({i+1}/{num_readings})")

                # Wait for the response and process it
                self.serial_thread.data_received.connect(self.display_response)

                # Save after each reading
                self.save_data_custom(i, filename_prefix)

                # Wait a bit before the next command (adjust as needed)
                time.sleep(1)

            self.response_textbox.append("Finished sending commands.")

    def save_data_custom(self, index, filename_prefix):
        """Save the data after each reading with a unique filename."""
        if not self.data:
            self.response_textbox.append(f"No data to save for acquisition {index+1}.")
            return

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{filename_prefix}_acquisition_{index+1}_{timestamp}.csv"
        ...
        try:
            ...
            self.response_textbox.append(f"Data saved to {filename}")
        except Exception as e:
