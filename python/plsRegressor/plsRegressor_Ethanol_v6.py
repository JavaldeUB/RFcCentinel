...
def optimise_pls_cv_manual(X, y, n_comp):
...
def optimise_polypls_cv_manual(X, y, n_comp, degree):
...
# Test with 40 components using training data
r2s = []
mses = []
rpds = []
...
for n_comp in xticks:
    print(f"Number of LV used: {n_comp}")
    r2, mse, rpd = optimise_polypls_cv_manual(X_train, y_train, n_comp, 3)  # Perform optimisation on training data
    r2s.append(r2)
    mses.append(mse)
    rpds.append(rpd)
