...
df[numeric_cols.columns] = numeric_cols.apply(np.log10)
df2 = df.T
X = df2.iloc[1:,:]
y = df2.iloc[0,1:]
...
y = pd.Series(y, index=X.columns) 
exclude_mask = y.isin(labels_to_leave_out)
X_ex = X.loc[:, ~exclude_mask] 
y_ex = y[~exclude_mask]
X_test, y_test = X.loc[:, exclude_mask] , y[exclude_mask]
# Randomize the training dataset
np.random.seed(42)
data_combined = pd.concat([X_ex.T, y_ex], axis=1)
shuffled_data = data_combined.sample(frac=1).reset_index(drop=True)
X_train = shuffled_data.iloc[:, :-1].T
...
param_grid = {
    'poly__degree': [1],
    'pls__n_components': [2,3,4,5,6,7,8,9,10]
}
...
pipe = Pipeline([
    ...
    ('pls', PLSRegression())         # PLS regression
])

grid_search = GridSearchCV(
    estimator=pipe,
    param_grid=param_grid,
    cv=logo.split(X_train.T, y_train, groups=groups),  # LOGO split
    scoring='neg_mean_squared_error',  # Use negative MSE as the metric
    n_jobs=-1,  # Parallel processing
    verbose=1   # Verbosity level for debugging
)
