...
rowsToLeft = find(abs(freqDev*1000)<24);
X_autoS = X_autoS(rowsToLeft,:);
X_bc = X_bc(rowsToLeft,:);
yp = yp(rowsToLeft);
counts = histcounts(yp.', uniqueNumbers(end:-1:1));
minCount = min(counts);
...
figure;
scatter(score(:, 1), score(:, 2), 50, yp, 'filled');
