...
clear opts
%degree=input("Order of the polynomial regressor: ");
%%
...
%%
%Autoscaling and log transformation
X_mean = mean(Xp);
X_std = std(Xp);
%X = Xp;
X = (Xp - X_mean)./std(Xp); %Con espectros al ppio no se estandariza con 
...
Y_pred_test = polypred(X_test,betaFinal,XLF,YLF,XWF,optimalComponents);
...
[~,idxVipFeat]=find(vipOptimal>=0.8);
figure;
fAxis = freqAxis./1e9;
...
angle_upper = atand(dy_upper / dx)-8; % Angle of upper LoA in degrees
angle_lower = atand(dy_lower / dx)-8; % Angle of lower LoA in degrees
...
Y_pred_interp = polypred(Xinterp,betaFinal,XLF,YLF,XWF,optimalComponents);
