...
freqAxis = dataBase(1,41:end-1);
y = dataBase(2:end,end);
X = dataBase(2:end,41:end-1);
% Getting the relevant adata from baseline
baselines = baselineData(2:end,41:end-1);
...
X_mean = mean(20.*log10(1+abs(X_bc)));
X_std = std(20.*log10(abs(X_bc)));
X_autoS = (20.*log10(1+abs(X_bc)));%-X_mean)./X_std;
...
if plt==1
    ...
    for nCls = 1:length(clusters)
        ...
        if length(cluster)>1
            % Define the rectangle coordinates
            rectangle('Position',[fAxis(min(cluster)) -11 fAxis(max(cluster))-fAxis(min(cluster)) 11],'EdgeColor','r','FaceColor',[1, 0, 0, 0.5])
        end
    end
     ylim([-11 0.5]);
end
...
%% Feature extraction
% Getting the features from the transformed matrix
Xfeat_Test = X_autoS(rowsEV,idxVipFeat);
yfeat_Test = yp(rowsEV);
Xfeat_Train = X_autoS(rowsLeft,idxVipFeat);
yfeat_Train = yp(rowsLeft);
...
% Split Data into training and internal validation
cv = cvpartition(groups, 'Holdout', 0.2);
X_train = Xfeat_Train(training(cv), :);
Y_train = yfeat_Train(training(cv), :);
groups_train = groups(training(cv));
X_test = Xfeat_Train(test(cv), :);
Y_test = yfeat_Train(test(cv), :);

%% LOGO Cross-Validation after Feature Extraction
% preparing the control variables
uniqueGroups = unique(groups_train);
maxComponents = 41; 
mseCV = zeros(maxComponents, 1);
...
disp('%%%% PERFORMANCE DURING EXTERNAL VALIDATION %%%%%%%%%%%%%%%%%%%%%%');
%Calculation of the prediction
Y_pred_test = [ones(size(Xfeat_Test, 1), 1), Xfeat_Test] * betaFinal;
mseTest = sqrt(mean((yfeat_Test - Y_pred_test).^2));
RSSinterp = sum((yfeat_Test - Y_pred_test).^2);
TSSinterp = sum((yfeat_Test - mean(yfeat_Test)).^2);
r2interp = 1 - (RSSinterp / TSSinterp);
disp(['Test RMSEP: ', num2str(mseTest)]);
disp(['Test R^{2}: ', num2str(r2)]);
%Bland-Altman+model
minVal = min([yfeat_Test; Y_pred_test]);
maxVal = max([yfeat_Test; Y_pred_test]);
% Calculate Bias and Limits of Agreement
bias = mean(Y_pred_test - yfeat_Test); % Mean difference
LoA = 1.96 * std(Y_pred_test - yfeat_Test); % Limits of agreement (±1.96 * SD)
% Add Labels for LoA Lines
upperLoA = bias + LoA; % Upper LoA value
lowerLoA = bias - LoA; % Lower LoA value
disp(['Test Upper LoA: ', num2str(upperLoA), ' (+1.96SD)']);
